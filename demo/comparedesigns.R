...
listofdesigns<-
  list(
    ...
    Design2b=list(draw=function(){s1<-stratsample(X$REGION,(a*c(table(X$REGION))/N))
    s2<-(1:N)[-s1][stratsample(X$REGION[-s1],0**c(table(X$REGION))+round((b*15)/nrow(Nh)))];c(s1,s2)},args=list(ids=~1,strata=~REGION,fpc=~Nh)),
    Design3a=list(draw=function(){stratsample(X$STRATA2,(n*c(table(X$STRATA2))/N))},args=list(ids=~1,strata=~STRATA2,fpc=~Nh2)),
    Design3b=list(draw=function(){s1<-stratsample(X$STRATA2,(a*c(table(X$STRATA2))/N))
    s2<-(1:N)[-s1][stratsample(X$STRATA2[-s1],0**c(table(X$STRATA2))+round((b*15)/nrow(Nh2)))];c(s1,s2)},args=list(ids=~1,strata=~STRATA2,fpc=~Nh2)))
set.seed(1)
